...
---
> 📌 **Кто здесь впервые?**
> - **Владелец/человек, который запускает сервис** → читай [`docs/LAUNCH.md`](docs/LAUNCH.md) — пошаговая инструкция «с самого начала» (с нуля, на пальцах, все шаги и кнопки).
> - **Агент / разработчик / «я всё забыл»** → читай [`PROJECT_STATE.md`](PROJECT_STATE.md) — полная карта проекта: что реализовано, как устроено, правила, текущая стадия, что делать дальше. После неё песочница не важна: весь контекст восстанавливается из репо.
>
> Технический деплой: [`deploy/README.md`](deploy/README.md) · Концепция и риски: [`docs/PLAN.md`](docs/PLAN.md)
...
## Структура
```
PROJECT_STATE.md  — 🧠 карта памяти: текущая стадия, всё реализованное, правила, «что дальше»
README.md         — этот файл
...
server/           — Node.js 22 / Express + встроенный node:sqlite + Grammy (бот)
  src/routes/     — auth, pay (ЮKassa + mock), account, devices (2+докупка, блокировка), webhooks
  src/vpn/        — провайдеры: mock (демо VLESS+Reality) и 3x-ui (реальный VPS)
  src/bot.js      — TG-бот: меню, покупка СБП, QR по устройствам, /block /unblock, AI-поддержка
  src/ai.js       — нейросетевая поддержка (KB + данные клиента + FAQ-фолбэк)
...
docs/             — LAUNCH.md (инструкция запуска с нуля), PLAN.md (концепция, риски),
                    ANALYSIS.md (24hype + конкуренты)
