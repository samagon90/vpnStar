...
function makeVerdict(d) {
  ...
  if (p) {
    if (tunnelOk) {
      v.push('Туннель РАБОТАЕТ: сайты грузятся через защищённый канал. Если в браузере не грузится — нажми в приложении кнопку «🌐 Системный прокси: вкл», затем перезапусти браузер (Chrome/Edge подхватывают прокси без перезапуска, Firefox — только если стоит «Использовать системные настройки прокси»).');
      if (d.t7 && !d.t7.ok) {
      ...
      v.push('СЕРВЕР НЕДОСТИЖИМ ИЗ ВАШЕЙ СЕТИ (таймаут прямого подключения): оператор/сеть не пропускает пакеты до сервера. Нужна смена протокола/узла.');
    ...
    if (d.t3 && !d.t3.ok) v.push('DNS на вашем ПК/сети работает плохо: ' + d.t3.extra + ' (если ОС не находит обычные сайты — проверьте DNS роутера/ОС, рекомендуем 1.1.1.1 или 8.8.8.8; через туннель это не влияет, DNS резолвит сервер)');
  }
  if (d.t6 && d.t6.ok) {
