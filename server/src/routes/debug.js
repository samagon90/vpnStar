// Диагностика: серверные проверки (с RU-сервера) для страницы /debug.html
...
router.get('/check', async (req, res) => {
  ...
  if (!DE_HOST) {
    return res.json({ ...out, error: 'XUI_HOST/XUI_PORT не настроены' });
  }

  // 1) TCP до порта VPN
  out.tcp = await tcpProbe(DE_HOST, DE_PORT, 5000);

  // 2) Параметры инбоунда из панели (сравнить с тем, что в профиле пользователя)
  try {
